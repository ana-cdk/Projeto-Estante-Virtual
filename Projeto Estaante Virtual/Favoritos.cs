...
namespace Projeto_Estante_Virtual
{
    public partial class Favoritos : Form
    {
        ...
        public Favoritos(Guid usuarioSelecionado)
        {
            InitializeComponent();
        ...
        {
            DTLivro.DataSource = _livroDAL.ObterFav(usuarioSelecionado);
            btRemover.Enabled = false;
            _livroAtual = null;
            DTLivro.ClearSelection();
        }

        private void Favoritos_Shown(object sender, EventArgs e)
        {
            InicializarFavorito();
            DTLivro.ClearSelection();
            _carregando = false;
